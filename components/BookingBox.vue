<template>
    <div class="booking-box-wrapper">
        <div class="booking-box__button" v-on:click.prevent="show_booking = !show_booking"  :class="{ 'visible': show_booking }" v-if="dates.length > 0">
            <span class="booking-box__pre-title" v-html="getText(checkEventLabel('class/singleEvent/bookingBox/nextClass'))"></span>
            ...
            <a class="booking-box__booking__button" href="#"  :disabled="isSaveDisabled" v-html="getText(checkEventLabel('class/singleEvent/bookingBox/bookNowButton'))"></a>
            </template>
        ...
        <div class="booking-box" :class="{ 'booking-box--visible': show_booking }">
            ...
            <template v-if="dates.length > 0">
                ...
                <span class="booking-box__price"><template v-if="price  === 0">{{ getText(checkEventLabel('class/singleEvent/bookingBox/priceFree')) }}</template><template v-else>{{price | currency(tenantCurrency)}}</template></span>
                <span class="booking-box__availability"><template v-if="next_class_capacity > 1">{{ getText(checkEventLabel('class/singleEvent/bookingBox/spotsAvailableMany'), { spots: next_class_capacity }) }}</template><template v-else-if="next_class_capacity > 0">{{ getText(checkEventLabel('class/singleEvent/bookingBox/spotsAvailableOne')) }}</template><template v-else>{{ getText(checkEventLabel('class/singleEvent/bookingBox/spotsAvailableNone')) }}</template></span>
                <a class="booking-box__booking-button" href="#" v-on:click.prevent="openModal(next_class.ts, next_class.dr)" :disabled="isSaveDisabled">{{ getText(checkEventLabel('class/singleEvent/bookingBox/bookNowButton')) }}</a>
                <template v-if="dates.length > 1 && !sold_out">
                    <span class="booking-box__or">or</span>
                    <span class="booking-box__choose" v-on:click.prevent="chooseDate = !chooseDate" v-html="getText(checkEventLabel('class/singleEvent/bookingBox/chooseDifferentDate'))"></span>
...
<script>
    ...
    export default {
        ...
        props: [ 'dates_next', 'dates_available', 'capacity', 'classNextDates', 'price', 'availability', 'availabilityRequest', 'tz', 'language', 'ts', 'is_event' ],
        computed: {
            sold_out: function(){
